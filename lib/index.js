...
function plugin(options) {
    ...
    return function(files, metalsmith, done) {
        ...
        for (var i = 0; i < fileNames.length; i++) {
            ...
            if (data.seo.description == 'byline') {
                data.seo.description = data.byline.replace(/<[^>]+>/ig, '').substring(0, 160);
            }
            else {
                data.seo.description = data.seo.description || options.seo.description;
            }

            // Fixes and adds in our own flavor for keywords.
            if (data.seo.keywords == 'tags') {
                data.seo.keywords = data.tags
            }
            else {
                data.seo.keywords = data.seo.keywords || options.seo.keywords;
            }
